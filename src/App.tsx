...
const CALCULATORS = [
  ...
  { id:'sofa', title:'SOFA – fallo orgánico secuencial',
  summary:'Subscores y total (resp, coag, hígado, CV, SNC, riñón).',
...
export default function App(){
  ...
  return (<div className='min-h-screen w-full bg-gradient-to-b from-white to-slate-50'><header className='sticky top-0 z-30 backdrop-blur bg-white/70 border-b'><div className='max-w-6xl mx-auto flex items-center gap-3 p-3'><HeartPulse className='h-6 w-6'/><div className='flex-1'><h1 className='text-xl font-semibold'>Críticos Cardiovasculares</h1><p className='text-xs text-slate-500'>Hospital Ramón y Cajal• v1.0</p></div><div className='flex items-center gap-2 w-full max-w-md'><div className='relative w-full'><Search className='absolute left-2 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400'/><Input value={q} onChange={(e)=>setQ(e.target.value)} placeholder='Buscar herramienta...' className='pl-8'/></div><Button variant='outline' size='icon' title='Ajustes'><Settings className='h-4 w-4'/></Button></div></div></header><main className='max-w-6xl mx-auto p-4 sm:p-6 space-y-8'>
...
function SOFATool({goHome}:{goHome:()=>void}) {
  return (
    <div className='min-h-screen w-full bg-white'>
      <header className='border-b sticky top-0 bg-white/70 backdrop-blur z-30'>
        <div className='max-w-3xl mx-auto flex items-center justify-between p-3'>
